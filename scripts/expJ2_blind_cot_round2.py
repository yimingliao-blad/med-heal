...
OUT_ROOT = PROJECT_ROOT / "runs" / "expJ2_blind_cot_round2"
...
R2_SYS = "You check whether a set of claims is consistent with a discharge note. You are NOT shown the original answer; judge the claims on their own, reasoning carefully step by step."
...
Check EACH claim against the discharge note. For EACH claim, reason step by step: find what the note says about it, then decide SUPPORTS / CONTRADICTS / SILENT. Treat SILENT as consistent — do not call a claim inconsistent just because the note does not restate it; only a claim the note clearly CONTRADICTS, or a required fact the note clearly states differently, counts as inconsistent.

After reasoning through all claims, end with exactly this section:
INCONSISTENT: for each claim the note clearly contradicts (or a required fact stated differently), give the claim and what the note actually says. Write "none" if every claim is consistent and together they correctly answer the question."""


def round2_consistency(row, claims, port) -> dict[str, Any]:
    raw = P2.vllm_chat(R2_SYS, R2_CONSISTENCY.format(note=row["note"][:24000], question=row["question"], claims=claims[:3000]), port, 900, 0.0, tag="r2.consistency_cot")
...
def main():
    ...
    set_ledger(out_dir / "llm_calls.jsonl", script="expJ2_blind_cot_round2", served=served, args=vars(args))
