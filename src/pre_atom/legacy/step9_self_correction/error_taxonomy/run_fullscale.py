#!/usr/bin/env python3
"""
Full-scale pipeline: all 109 wrong + 150 sampled correct.
Reuses existing 100-item detection results.
Detection → Correction (P1+pool) → Verdict (V1) → GPT-4o eval.
Resume-safe.

Usage:
    python run_fullscale.py --port 8003
"""
...
# ============================================================
# PROMPTS (same as run_all_detection_5fold.py + run_correction_verdict_5fold.py)
# ============================================================
...
def main():
    ...
    # Build item set: all 109 wrong + ALL 853 correct
    all_wrong = all_df[all_df["binary_correct"]==0]
    all_correct = all_df[all_df["binary_correct"]==1]
    target_items = []
    for _, row in all_wrong.iterrows():
        target_items.append({"idx": int(row["idx"]), "fold": int(row["fold"]), "label": "wrong", "row": row})
    for _, row in all_correct.iterrows():
        target_items.append({"idx": int(row["idx"]), "fold": int(row["fold"]), "label": "correct", "row": row})

    n_w = sum(1 for t in target_items if t["label"] == "wrong")
    n_c = sum(1 for t in target_items if t["label"] == "correct")

    # Load existing detection results to reuse
    existing_det = {}
    existing_file = OUTPUT_DIR / "all_detection_5fold.json"
    if existing_file.exists():
        for r in json.load(open(existing_file)):
            existing_det[(r["fold"], r["idx"])] = r["det"]

    save_file = OUTPUT_DIR / "fullscale_results.json"
    results = []
    done_keys = set()
    if save_file.exists():
    ...
    print(f"Full-scale: {n_w} wrong + {n_c} correct = {len(target_items)} items", flush=True)
    print(f"Existing detection to reuse: {len(existing_det)}", flush=True)
    print("Pipeline: S1 detect → P1+pool correct → V1 verdict", flush=True)
    print("=" * 70, flush=True)
    ...
    for ti in target_items:
        ...
        # DETECTION — reuse or run
        det_key = (ti["fold"], ti["idx"])
        if det_key in existing_det:
            det = existing_det[det_key]
        else:
            det = {}
        ...
        detected_types = [k for k in ["contra", "qmis", "omis"] if det.get(k, {}).get("verdict") == "INCORRECT"]
        ...
        if not any_detected:
            results.append({
                ...
                "final_eval": eval_orig, "action": "keep",
            })
            with open(save_file, "w") as f: json.dump(results, f)
            if len(results) % 20 == 0:
                wd = sum(1 for r in results if r["label"]=="wrong" and r.get("detected"))
                cd = sum(1 for r in results if r["label"]=="correct" and r.get("detected"))
                wt = sum(1 for r in results if r["label"]=="wrong")
                ct = sum(1 for r in results if r["label"]=="correct")
                print(f"  [{len(results)}/{len(target_items)}] det w={wd}/{wt} c={cd}/{ct} ${spending['cost']:.2f}", flush=True)
            continue

        # Most critical error (qmis > contra > omis)
        mc = None
        mc_type = "OMISSION"
        ...
        # CORRECTION (P1+pool, type-routed)
        pool_ex = retrieve_pool(mc["error_statement"], mc_type, ti["fold"])
        if pool_ex:
            if mc_type == "CONTRADICTION":
                ...
                msg = COR_OMISSION
            fmt = {"note": note, "question": row["question"],
            ...
            try:
                corrected = vllm_gen(build_chatml("You are a medical expert.", msg.format(**fmt)), max_tokens=512, temperature=1.0)
            except:
                corrected = vllm_gen(build_chatml("You are a medical expert.",
                    COR_NOPOOL.format(note=note, question=row["question"], error_type=mc_type,
                        error_statement=mc["error_statement"][:200], correct_statement=mc["correct_statement"][:200])),
        ...
        # GPT-4o eval (only if V1 accepted)
        if accept:
            ev_cor = gpt4o_eval(note, row["question"], gt, corrected)
            final_eval = ev_cor
            action = "corrected"
        else:
            ev_cor = -1  # not evaluated
        ...
        entry = {
            ...
            "det_types": detected_types, "error_type": mc_type,
            "v1_accept": accept, "eval_corrected": ev_cor,
            "final_eval": final_eval, "action": action,
        ...
        if len(results) % 10 == 0:
            ...
            wt = sum(1 for r in results if r["label"]=="wrong")
            ct = sum(1 for r in results if r["label"]=="correct")
            fixes = sum(1 for r in results if r["label"]=="wrong" and r.get("action")=="corrected" and r.get("final_eval")==1)
            brks = sum(1 for r in results if r["label"]=="correct" and r.get("action")=="corrected" and r.get("final_eval")==0)
            print(f"  [{len(results)}/{len(target_items)}] det w={wd}/{wt} c={cd}/{ct} | fix={fixes} brk={brks} ${spending['cost']:.2f}", flush=True)
    ...
    det_w = [r for r in all_w if r.get("detected")]
    det_c = [r for r in all_c if r.get("detected")]
    fixes = [r for r in all_w if r.get("action")=="corrected" and r.get("final_eval")==1]
    brks = [r for r in all_c if r.get("action")=="corrected" and r.get("final_eval")==0]

    print(f"\n{'='*70}", flush=True)
    print(f"FULL-SCALE RESULTS: {len(all_w)} wrong + {len(all_c)} correct", flush=True)
    print(f"{'='*70}", flush=True)
    print(f"Detection: TP={len(det_w)}/{len(all_w)} ({100*len(det_w)/len(all_w):.0f}%) FP={len(det_c)}/{len(all_c)} ({100*len(det_c)/len(all_c):.0f}%)", flush=True)

    v1_acc_w = sum(1 for r in det_w if r.get("v1_accept"))
    v1_acc_c = sum(1 for r in det_c if r.get("v1_accept"))
    print(f"V1 accepted: TP={v1_acc_w}/{len(det_w)} FP={v1_acc_c}/{len(det_c)}", flush=True)
    print(f"Fixes: {len(fixes)}", flush=True)
    print(f"Breaks: {len(brks)}", flush=True)
    print(f"Net: {len(fixes) - len(brks):+d}", flush=True)

    # Projection to 962
    total = 962; total_w = 109; total_c = 853
    base_acc = total_c / total
    # Fix rate among all wrong items (including non-detected)
    fix_rate = len(fixes) / len(all_w)
    brk_rate = len(brks) / len(all_c)
    proj_fix = fix_rate * total_w
    proj_brk = brk_rate * total_c
    proj_acc = base_acc + (proj_fix - proj_brk) / total

    print(f"\nProjection to 962 items:", flush=True)
    print(f"  Fix rate: {100*fix_rate:.1f}% → {proj_fix:.0f} fixes", flush=True)
    print(f"  Break rate: {100*brk_rate:.1f}% → {proj_brk:.0f} breaks", flush=True)
    print(f"  Accuracy: {100*base_acc:.2f}% → {100*proj_acc:.2f}% ({100*(proj_acc-base_acc):+.2f}pp)", flush=True)
    ...
    for fold in range(5):
        ...
        f_fix = sum(1 for r in fw if r.get("action")=="corrected" and r.get("final_eval")==1)
        f_brk = sum(1 for r in fc if r.get("action")=="corrected" and r.get("final_eval")==0)
        f_det_w = sum(1 for r in fw if r.get("detected"))
        f_det_c = sum(1 for r in fc if r.get("detected"))
        print(f"  Fold {fold}: det w={f_det_w}/{len(fw)} c={f_det_c}/{len(fc)} | fix={f_fix} brk={f_brk} net={f_fix-f_brk:+d}", flush=True)
